        ...
        if redis.get(f'{args["phone_number"]}'):
            ...
            try:
                ...
                if response.body.code == 'OK':
                    redis.set(f'{args["phone_number"]}', code, ex=300)
                    logger.debug(f'手机号{args["phone_number"]}发送验证码成功')
                    return make_response(jsonify(code=200, message='发送成功'), 200)
                else:
                    return make_response(jsonify(code=400, message='发送失败:' + response.body.message), 400)
